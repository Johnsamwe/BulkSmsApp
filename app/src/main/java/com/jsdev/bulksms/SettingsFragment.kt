...
class SettingsFragment : Fragment(R.layout.fragment_settings) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<MaterialButton>(R.id.btnViewCrash).setOnClickListener { showLastCrash() }

        updatePinStatus(view)
    }

    private fun showLastCrash() {
        val report = CrashHandler.getLastCrash(requireContext())
        if (report == null) {
            Toast.makeText(requireContext(), "Hakuna crash iliyorekodiwa — nzuri!", Toast.LENGTH_SHORT).show()
            return
        }
        AlertDialog.Builder(requireContext())
            .setTitle("Ripoti ya Crash ya Mwisho")
            .setMessage(report)
            .setPositiveButton("Nakili") { _, _ ->
                val cm = requireContext().getSystemService(android.content.Context.CLIPBOARD_SERVICE) as android.content.ClipboardManager
                cm.setPrimaryClip(android.content.ClipData.newPlainText("crash_report", report))
                Toast.makeText(requireContext(), "Imenakiliwa", Toast.LENGTH_SHORT).show()
            }
            .setNegativeButton("Futa") { _, _ ->
                CrashHandler.clearLastCrash(requireContext())
                Toast.makeText(requireContext(), "Imefutwa", Toast.LENGTH_SHORT).show()
            }
            .setNeutralButton("Funga", null)
            .show()
    }
